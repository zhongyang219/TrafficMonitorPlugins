...
bool CCommon::GetURLBinary(const std::wstring& url, std::vector<BYTE>& result, LPCTSTR user_agent)
{
    bool succeed{ false };
    result.clear();
    CInternetSession* pSession{};
    ...
    try
    {
        ...
        if (dwStatusCode == HTTP_STATUS_OK)
        {
            ...
            {
                result.insert(result.end(), buffer, buffer + nRead);
            }
            succeed = true;
        }
        pfile->Close();
    ...
    {
        CCommon::WriteLog(L"request binary fail!", g_data.m_log_path.c_str());
        if (pfile != nullptr)
        {
            pfile->Close();
            delete pfile;
