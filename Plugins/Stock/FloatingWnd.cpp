...
int CFloatingWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CRect maBtnRect(bollBtnRect.left - rightBtnWidth, g_data.RDPI(2), bollBtnRect.left, g_data.RDPI(2) + btnHeight);
	m_btnMA.Create(_T("MA"), WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON | BS_FLAT, maBtnRect, this, IDC_MA_BTN);

	CRect trendBtnRect(maBtnRect.left - rightBtnWidth, g_data.RDPI(2), maBtnRect.left, g_data.RDPI(2) + btnHeight);
	m_btnTrend.Create(_T("K线"), WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON | BS_FLAT, trendBtnRect, this, IDC_TREND_BTN);

	CRect t0BtnRect(trendBtnRect.left - rightBtnWidth, g_data.RDPI(2), trendBtnRect.left, g_data.RDPI(2) + btnHeight);
	...
	m_btnIndicatorMACD.Create(_T("MACD"), WS_CHILD | BS_PUSHBUTTON | BS_FLAT, CRect(0, 0, 0, 0), this, IDC_INDICATOR_MACD_BTN);
	m_btnIndicatorKDJ.Create(_T("KDJ"), WS_CHILD | BS_PUSHBUTTON | BS_FLAT, CRect(0, 0, 0, 0), this, IDC_INDICATOR_KDJ_BTN);
	m_btnIndicatorWR.Create(_T("W&&R"), WS_CHILD | BS_PUSHBUTTON | BS_FLAT, CRect(0, 0, 0, 0), this, IDC_INDICATOR_WR_BTN);
	m_btnIndicatorRSI.Create(_T("RSI"), WS_CHILD | BS_PUSHBUTTON | BS_FLAT, CRect(0, 0, 0, 0), this, IDC_INDICATOR_RSI_BTN);
...
void CFloatingWnd::DrawTimelineGridLines(CDC& memDC, const TimelineDrawContext& ctx)
{
	...
	if (ctx.maxPrice > 0 && ctx.minPrice > 0 && ctx.maxPrice > ctx.minPrice && ctx.niceStep > 0)
	{
		...
		{
			double labelPrice = ctx.minPrice + i * ctx.niceStep;
		...
		&& ctx.realtimeData.prevClosePrice >= ctx.minPrice && ctx.realtimeData.prevClosePrice <= ctx.maxPrice)
	{
		...
		int prevCloseY = ctx.priceChartTop + ctx.priceChartHeight - static_cast<int>((ctx.realtimeData.prevClosePrice - ctx.minPrice) * ctx.unitY);
...
void CFloatingWnd::DrawTimelineHoverOverlay(CDC& memDC, const TimelineDrawContext& ctx)
{
	...
	int dotY = ctx.priceChartTop + ctx.priceChartHeight - static_cast<int>((item.price - minPrice) * unitY);
	...
	if (!m_isKLineMode)
	{
		...
		int avgDotY = ctx.priceChartTop + ctx.priceChartHeight - static_cast<int>((avgPrice - minPrice) * unitY);
...
void CFloatingWnd::OnPaint()
{
	...
	if (!m_isOverviewMode)
	{
		...
		if (!timelinePoint.empty())
		{
			...
			// MACD/KDJ/W&R/RSI指标按钮（MACD图左侧Y轴预留区域，四等分排列）
			{
				int btnW = yAxisWidth - g_data.RDPI(4);
				int btnH = g_data.RDPI(16);
				int btnX = g_data.RDPI(2);
				int macdH = ctx.macdChartHeight;
				int btn1Y = origMacdTop + titleH + (macdH / 4 - btnH) / 2;
				int btn2Y = origMacdTop + titleH + macdH / 4 + (macdH / 4 - btnH) / 2;
				int btn3Y = origMacdTop + titleH + macdH * 2 / 4 + (macdH / 4 - btnH) / 2;
				int btn4Y = origMacdTop + titleH + macdH * 3 / 4 + (macdH / 4 - btnH) / 2;
				if (m_btnIndicatorMACD.GetSafeHwnd())
				{
					m_btnIndicatorMACD.SetWindowPos(nullptr, btnX, btn1Y, btnW, btnH, SWP_NOZORDER | SWP_NOREDRAW);
					if (!m_btnIndicatorMACD.IsWindowVisible())
						m_btnIndicatorMACD.ShowWindow(SW_SHOW);
					m_btnIndicatorMACD.SetButtonStyle(m_timelineIndicator == TimelineIndicator::MACD ? BS_DEFPUSHBUTTON : BS_FLAT, TRUE);
				}
				if (m_btnIndicatorKDJ.GetSafeHwnd())
				{
					m_btnIndicatorKDJ.SetWindowPos(nullptr, btnX, btn2Y, btnW, btnH, SWP_NOZORDER | SWP_NOREDRAW);
					if (!m_btnIndicatorKDJ.IsWindowVisible())
						m_btnIndicatorKDJ.ShowWindow(SW_SHOW);
					m_btnIndicatorKDJ.SetButtonStyle(m_timelineIndicator == TimelineIndicator::KDJ ? BS_DEFPUSHBUTTON : BS_FLAT, TRUE);
				}
				if (m_btnIndicatorWR.GetSafeHwnd())
				{
					m_btnIndicatorWR.SetWindowPos(nullptr, btnX, btn3Y, btnW, btnH, SWP_NOZORDER | SWP_NOREDRAW);
					if (!m_btnIndicatorWR.IsWindowVisible())
						m_btnIndicatorWR.ShowWindow(SW_SHOW);
					m_btnIndicatorWR.SetButtonStyle(m_timelineIndicator == TimelineIndicator::WR ? BS_DEFPUSHBUTTON : BS_FLAT, TRUE);
				}
				if (m_btnIndicatorRSI.GetSafeHwnd())
				{
					m_btnIndicatorRSI.SetWindowPos(nullptr, btnX, btn4Y, btnW, btnH, SWP_NOZORDER | SWP_NOREDRAW);
					if (!m_btnIndicatorRSI.IsWindowVisible())
						m_btnIndicatorRSI.ShowWindow(SW_SHOW);
					m_btnIndicatorRSI.SetButtonStyle(m_timelineIndicator == TimelineIndicator::RSI ? BS_DEFPUSHBUTTON : BS_FLAT, TRUE);
				}
			}
		...
		// 绘制底部大盘指数状态栏
		{
			...
			// 三个指数代码：上证指数、中证银行、恒生科技
			const std::wstring indexCodes[] = { L"sh000001", L"sz399986", L"rt_hkHSTECH" };
			...
			{
				auto stockData = g_data.GetStockData(indexCodes[i]);
				int colX = i * colWidth;
				...
				{
					CString nameStr;
					if (i == 0) nameStr = _T("上证指数");
					else if (i == 1) nameStr = _T("中证银行");
					else nameStr = _T("恒生科技");
...
void CFloatingWnd::DrawOrderBook(CDC& memDC, int left, int right, int height, const STOCK::StockInfo& stockInfo, const std::vector<STOCK::KLinePoint>& klineData)
{
	...
	auto DrawNetRatioBarText = [&](int x, int y, int h, const CString& text) {
		...
		memDC.TextOut(x, y + max(0, (h - memDC.GetTextExtent(text).cy) / 2), text);
	...
	int wbBarY = topOffset;
	int wbBarH = rowHeight;
	...
	DrawNetRatioBarText(wbBarX + g_data.RDPI(3), wbBarY, wbBarH, wbTxt);
	...
	if (barW > 0)
	{
		...
		CString netRatioTxt;
		netRatioTxt.Format(_T("%.2f%s %s%s"), std::abs(netRatio), netRatioTrend.GetString(), diffSign.GetString(), netDiffStr.GetString());
		DrawNetRatioBarText(barX + g_data.RDPI(3), barY, barH, netRatioTxt);
	}
	...
	for (int i = 0; i < 4; i++)
	{
		...
		if (hasData)
		{
			...
			periodTxt.Format(_T("%.2f%s %s%s"), std::abs(ratio), ratioTrend.GetString(), diffSign.GetString(), diffStr.GetString());
		}
		else
		{
			periodTxt = _T("-- --");
		...
		DrawNetRatioBarText(periodBarX + g_data.RDPI(3), periodBarY, periodBarH, periodTxt);
	}
...
void CFloatingWnd::UpdateModeButtons()
{
	if (m_btnTimeLine.GetSafeHwnd() && m_btnKLine.GetSafeHwnd())
	{
		...
		{
			m_btnT0.SetButtonStyle(m_showT0Markers ? BS_DEFPUSHBUTTON : BS_FLAT, TRUE);
		}
		...
		// MACD/KDJ/WR指标按钮在所有模式下显示（除总览模式外）
		if (m_btnIndicatorMACD.GetSafeHwnd())
		{
			if (!m_isOverviewMode)
				m_btnIndicatorMACD.ShowWindow(SW_SHOW);
